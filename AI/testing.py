...
UDP_IP = '192.168.43.114'
UDP_PORT = 5005
MAX_DGRAM = 1400  # Smaller packets to avoid frame drop
udp_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
try:
    while True:
        ...
        # Resize for UDP streaming only
        resized = cv2.resize(frame, (320, 240))
        _, encoded = cv2.imencode('.jpg', resized, [int(cv2.IMWRITE_JPEG_QUALITY), 60])
        img_data = encoded.tobytes()
        chunks = [img_data[i:i+MAX_DGRAM-8] for i in range(0, len(img_data), MAX_DGRAM-8)]
        for chunk in chunks:
            header = struct.pack('>II', frame_num, len(chunks))
        ...
        time.sleep(1 / 15)  # ~15 FPS for normal frames
